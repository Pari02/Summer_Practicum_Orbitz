...
for (i in 1:length(city_files))
{
  ...
  city_df <- read.csv(city_files[1], header = TRUE)
  ...
  {
    hotel <- hotel_city[1]
    hotel_filter <- which(sapply(lapply(1:nrow(city_rating), function(x) any(which(city_df$Hotel[x] %in% hotel)))
                                 , isTRUE))   
    ...
    for (x in 1:length(bigram_hash$bigram))
    {
      ...
      bigram_hash$rating_1[x] <- ifelse(length(r1$freq[r1$x == "TRUE"]) == 0, 0, r1$freq[r1$x == "TRUE"])
      ...
      bigram_hash$rating_2[x] <- ifelse(length(r2$freq[r2$x == "TRUE"]) == 0, 0, r2$freq[r2$x == "TRUE"])
      ...
      bigram_hash$rating_3[x] <- ifelse(length(r3$freq[r3$x == "TRUE"]) == 0, 0, r3$freq[r3$x == "TRUE"])
      ...
      bigram_hash$rating_4[x] <- ifelse(length(r4$freq[r4$x == "TRUE"]) == 0, 0, r4$freq[r4$x == "TRUE"])
      ...
      bigram_hash$rating_5[x] <- ifelse(length(r5$freq[r5$x == "TRUE"]) == 0, 0, r5$freq[r5$x == "TRUE"])
    }
    ...
    write.csv(bigram_hash, file.path(dirC, paste(hotel, "bigram_hash.csv", sep = "_")), row.names = FALSE)
  }
}
